...
export const CheckoutProvider = (props: {
  ...
  const onSubmitForm = async (data: CheckoutInfo) => {
    try {
      if (authUser) {
        ...
        await createCheckoutWithoutUser({
          variables: {
            data: {
              checkoutTokenId: data.token,
              firstName: data.firstName,
  ...
  useEffect(() => {
    if (checkout && !router?.pathname?.includes('/transaction')) {
      setTimeout(() => {
        router.push({
          pathname: '/[checkoutRequestId]/transaction',
          query: {
            ...router.query,
            id: checkout.id
          },
  ...
  useEffect(() => {
    setRequestId(router.query.checkoutRequestId as string)
  }, [router, setRequestId])
