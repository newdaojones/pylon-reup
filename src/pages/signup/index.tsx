...
import { useAgreement } from "@src/context/agreement";
import { useCheckout } from "@src/context/checkout";
import { GET_AGREEMENT_LINK } from "@src/utils/graphql";
import { stateList } from "@src/constants/state";
import { useRouter } from "next/router";


export default function Signup() {
  const router = useRouter()
  const { checkoutInfo, onCreateAccount, isLoading, loadingMessage, user } = useCheckout()
    ...
    <div className="flex flex-col gap-2">
      ...
      <div className="flex gap-2">
        <div className="flex-1">
          ...
          />
          {touched.dob && errors.dob && <div className='text-red-400 text-[12px] text-left'>{errors.dob as string}</div>}
        </div>
        <div className="flex-1">
